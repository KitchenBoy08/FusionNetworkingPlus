...
namespace Riptide
{
    ...
    public class Server : Peer
    {
        ...
        public void Start(ushort port, ushort maxClientCount, byte messageHandlerGroupId = 0, bool useMessageHandlers = true)
        {
            ...
            Heartbeat();
            IsRunning = true;
            RiptideLogger.Log(Utils.LogType.Info, LogName, $"Started on port {port}.");
        }
        ...
        public void Accept(Connection connection)
        {
            if (pendingConnections.Remove(connection))
                ...
                RiptideLogger.Log(Utils.LogType.Warning, LogName, $"Couldn't accept connection from {connection} because no such connection was pending!");
        }
        ...
        public void Reject(Connection connection, Message message = null)
        {
            if (message != null && message.ReadBits != 0)
                RiptideLogger.Log(Utils.LogType.Error, LogName, $"Use the parameterless 'Message.Create()' overload when setting rejection data!");

            if (pendingConnections.Remove(connection))
                Reject(connection, message == null ? RejectReason.Rejected : RejectReason.Custom, message);
            else
                RiptideLogger.Log(Utils.LogType.Warning, LogName, $"Couldn't reject connection from {connection} because no such connection was pending!");
        }
        ...
        private void Reject(Connection connection, RejectReason reason, Message rejectMessage = null)
        {
            ...
            RiptideLogger.Log(Utils.LogType.Info, LogName, $"Rejected connection from {connection}: {Helper.GetReasonString(reason)}.");
        }
        ...
        protected override void Handle(Message message, MessageHeader header, Connection connection)
        {
            switch (header)
            {
                ...
                case MessageHeader.Welcome:
                    ...
                    break;
                default:
                    RiptideLogger.Log(Utils.LogType.Warning, LogName, $"Unexpected message header '{header}'! Discarding {message.BytesInUse} bytes received from {connection}.");
                    break;
            }
        ...
        {
            if (message != null && message.ReadBits != 0)
                RiptideLogger.Log(Utils.LogType.Error, LogName, $"Use the parameterless 'Message.Create()' overload when setting disconnection data!");
            ...
            if (clients.TryGetValue(id, out Connection client))
            {
                ...
                RiptideLogger.Log(Utils.LogType.Warning, LogName, $"Couldn't disconnect client {id} because it wasn't connected!");
        }
        ...
        public void DisconnectClient(Connection client, Message message = null)
        {
            ...
            if (clients.ContainsKey(client.Id))
            {
                ...
                RiptideLogger.Log(Utils.LogType.Warning, LogName, $"Couldn't disconnect client {client.Id} because it wasn't connected!");
        }
        ...
        public void Stop()
        {
            ...
            StopTime();
            IsRunning = false;
            RiptideLogger.Log(Utils.LogType.Info, LogName, "Server stopped.");
        }
        ...
        private ushort GetAvailableClientId()
        {
            ...
            RiptideLogger.Log(Utils.LogType.Error, LogName, "No available client IDs, assigned 0!");
            return 0;
        }
        ...
        {
            RiptideLogger.Log(Utils.LogType.Info, LogName, $"Client {client.Id} ({client}) connected successfully!");
        ...
        {
            RiptideLogger.Log(Utils.LogType.Info, LogName, $"Client {connection} stopped responding before the connection was fully established!");
        ...
        protected virtual void OnMessageReceived(Message message, Connection fromConnection)
        {
            ...
            if (useMessageHandlers)
            {
                if (messageHandlers.TryGetValue(messageId, out MessageHandler messageHandler))
                    ...
                    RiptideLogger.Log(Utils.LogType.Warning, LogName, $"No message handler method found for message ID {messageId}!");
            }
        }
        ...
        {
            RiptideLogger.Log(Utils.LogType.Info, LogName, $"Client {connection.Id} ({connection}) disconnected: {Helper.GetReasonString(reason)}.");
